...
# VLANs where IPs are DHCP — ping by hostname (<name> field) instead of IP
...
def load_xml(path: str) -> dict[str, dict]:
    ...
    for dev in root.findall("device"):
        ...
        record = {
            ...
            "port":   g("port"),
            "vlan":   g("vlan"),
            "type":   g("type") or guess_type(name),  # explicit XML type wins
        }
        # Capture ALL extra fields from XML automatically (skip type, already handled)
        for child in dev:
