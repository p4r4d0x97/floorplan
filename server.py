...
def merge_devices(new_devs: dict):
    ...
    for mac, dev in new_devs.items():
        if mac in devices_by_mac:
            ...
            core_skip = {"removed"}
            updates = {k:v for k,v in dev.items() if k not in core_skip and existing.get(k)!=v}
            removed_keys = [k for k in existing if k not in core_skip and k not in dev]
            if removed_keys:
                for k in removed_keys: existing.pop(k,None)
