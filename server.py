...
import sys, os, json, time, threading, subprocess, platform
...
# ── Config ─────────────────────────────────────────────────────────────────────
PORT         = 8000
VLAN_PAUSE   = 15        # seconds pause between VLAN groups during a /api/ping call
LAYOUT_FILE  = Path(__file__).parent / "layout.json"
HISTORY_FILE = Path(__file__).parent / "history.json"
ALERTS_FILE  = Path(__file__).parent / "alerts.json"
KNOWN_FILE   = Path(__file__).parent / "known_macs.json"  # MACs ever seen in XML

# ── Stores ──────────────────────────────────────────────────────────────────────
devices_by_mac: dict[str, dict] = {}
status_store:   dict[str, str]  = {}   # mac -> "online"|"offline"|"no_info"
# history: mac -> {status, last_seen (epoch), last_changed (epoch), prev_status}
history_store:  dict[str, dict] = {}
known_macs:     set             = set()   # all MACs ever present in XML
store_lock = threading.Lock()
ping_lock  = threading.Lock()   # only one ping sweep at a time
...
def load_xml(path: str) -> dict[str, dict]:
    ...
    for dev in root.findall("device"):
        ...
        for child in dev:
            if child.tag not in CORE and child.text:
...
def ping_once(ip: str) -> bool:
    ...
    try:
        ...
        print(f"[ping-cmd] {' '.join(cmd)}")   # DEBUG — remove later
        ...
        print(f"[ping-out] exit={result.returncode} | {out.strip()[:120]}")  # DEBUG
        return result.returncode == 0
    except subprocess.TimeoutExpired:
        print(f"[ping] {ip} → subprocess timeout")
        return False
...
def do_ping(macs: list[str]) -> dict[str, str]:
    ...
    try:
        ...
        for i, (vlan, group) in enumerate(sorted(by_vlan.items())):
            batch = {}
            threads = []
            def probe(d, res=batch):
                ip = d.get("ip", "")
                res[d["mac"]] = ("online" if ping_once(ip) else "offline") if ip else "no_info"
            for d in group:
            ...
            print(f"[ping] VLAN {vlan:>4}  {len(group):>3} devs  online={online}")
            if i < len(by_vlan) - 1:
...
def merge_devices(new_devs: dict):
    ...
    - New MAC → add it
    - Existing MAC with changed fields → update those fields (ip, name, vlan, switch, port, type)
    ...
    for mac, dev in new_devs.items():
        if mac in devices_by_mac:
            ...
            fields = ["ip","name","vlan","switch","port","type"]
            updates = {f: dev[f] for f in fields if dev.get(f) != existing.get(f)}
            if updates:
                existing.update(updates)
                existing.pop("removed", None)
                changed.append(f"updated {existing.get('name',mac)}: {updates}")
