...
def collect_serial_and_second_nic(device: RawDevice, username: str, password: str, port: int) -> None:
    """
    Quick ping first — skip powershell entirely if device is unreachable.
    Single bundled powershell call: serial + all IPs + location folders
    in one SSH round trip.
    Filters: VLAN whitelist + hostname prefix (C0 or W).
    """
    if device.vlan not in SERIAL_NIC_VLAN_WHITELIST:
        return
    if not any(device.hostname.upper().startswith(p) for p in SERIAL_NIC_NAME_PREFIXES):
        return

    # Fast ping before opening SSH — saves ~5s per unreachable device
    if not ping(device.ip):
        device.online = False
        return
    ...
    try:
        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        client.connect(device.ip, port, username, password, timeout=5)

        # ── Single bundled powershell call ────────────────────────────────────
        # Collects: serial, all IPs, location folder presence
        # One round trip replaces what was previously 10+ separate calls
        folder_checks = "".join([
        ...
        _, stdout, _ = client.exec_command(f'powershell -Command "{ps_script}"')
        ...
        print(f"[!] Powershell collection failed for {device.ip}: {e}")
        device.online = False
    finally:
        if client:
            client.close()
...
def _collect_serial_wrapper(args: tuple) -> None:
    device, username, password, port = args
    collect_serial_and_second_nic(device, username, password, port)
...
def run_collection(username: str, password: str, port: int) -> list[RawDevice]:
    ...
    print("[→] Collecting serial / second NIC / location folders...")
    serial_args = [(d, username, password, port) for d in devices]
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        pool.map(_collect_serial_wrapper, serial_args)
...
class TagCollector:
    ...
    Location tags are collected in the bundled powershell call during
    collection so they already live in dev.tags before this runs.
    Add new tag sources by adding a method and registering it in collect_all().
    """
...
if __name__ == "__main__":
    ...
    mode      = ask_scan_mode()
    collector = TagCollector(username, password, port)
